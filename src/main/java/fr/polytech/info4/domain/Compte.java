...
public class Compte implements Serializable {
    ...
    @OneToMany(mappedBy = "estlivre")
    @Cache(usage = CacheConcurrencyStrategy.READ_WRITE)
    @JsonIgnoreProperties(value = { "montant", "plats", "estlivre" }, allowSetters = true)
    ...
    public Compte addCourse(Course course) {
        ...
        course.setEstlivre(this);
        return this;
    }

    public Compte removeCourse(Course course) {
        this.courses.remove(course);
        course.setEstlivre(null);
        return this;
    }

    public void setCourses(Set<Course> courses) {
        if (this.courses != null) {
            this.courses.forEach(i -> i.setEstlivre(null));
        }
        if (courses != null) {
            courses.forEach(i -> i.setEstlivre(this));
        }
