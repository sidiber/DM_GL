...
class CourseResourceIT {
    ...
    @Test
    @Transactional
    void getAllCoursesByLivreIsEqualToSomething() throws Exception {
        // Initialize the database
        courseRepository.saveAndFlush(course);
        Compte livre = CompteResourceIT.createEntity(em);
        em.persist(livre);
        em.flush();
        course.setLivre(livre);
        courseRepository.saveAndFlush(course);
        Long livreId = livre.getId();

        // Get all the courseList where livre equals to livreId
        defaultCourseShouldBeFound("livreId.equals=" + livreId);

        // Get all the courseList where livre equals to (livreId + 1)
        defaultCourseShouldNotBeFound("livreId.equals=" + (livreId + 1));
    }
